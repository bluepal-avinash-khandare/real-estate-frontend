...
// //   // Get current location to check if we're on auth pages
...
// //           {/* Logo with gradient accent - adjusted for left alignment */}
// //           <div className="flex items-center -ml-4"> {/* Added negative margin to pull left */}
// //             {/* Sidebar Toggle Button - adjusted positioning */}
// //             <button
...
// //             <Link to="/" className="flex items-center group ml-2"> {/* Reduced margin between button and logo */}
...
// // Premium Flaticon-style icons with enhanced styling
// const Icons = {
//   search: (
//     <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
//       <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" fill="#2C3E50" />
...
//   const location = useLocation();
//   const isAuthPage = location.pathname === '/login' || location.pathname === '/register' || location.pathname === '/register-choice';
...
const Navbar = () => {
  ...
  const [profileDropdownOpen, setProfileDropdownOpen] = useState(false);
  const [propertiesDropdownOpen, setPropertiesDropdownOpen] = useState(false);
  const [propertiesDropdownHover, setPropertiesDropdownHover] = useState(false);
  const [dashboardHover, setDashboardHover] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (propertiesRef.current && !propertiesRef.current.contains(event.target) && propertiesDropdownOpen) {
        setPropertiesDropdownOpen(false);
      }
    };

    if (profileDropdownOpen || propertiesDropdownOpen) {
      document.addEventListener('mousedown', handleClickOutside);
      document.addEventListener('touchstart', handleClickOutside);
    }
  ...
  }, [profileDropdownOpen, propertiesDropdownOpen]);

  // Toggle properties dropdown on click
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center space-x-6">
            ...
            <div
              ...
              className="relative"
              onMouseEnter={() => {
                setPropertiesDropdownHover(true);
                setPropertiesDropdownOpen(true);
              }}
              onMouseLeave={() => {
                setPropertiesDropdownHover(false);
            ...
            >
              <button 
                className="flex items-center text-gray-700 hover:text-[#16A085] font-medium transition-colors"
                onClick={togglePropertiesDropdown}
              >
                Properties {Icons.arrow}
              </button>

              {(propertiesDropdownOpen || propertiesDropdownHover) && (
                <div 
                  className="absolute left-0 mt-2 w-56 bg-white rounded-xl shadow-xl py-3 z-50 border border-gray-100 overflow-hidden"
                  onMouseEnter={() => setPropertiesDropdownHover(true)}
                  onMouseLeave={() => {
                    setPropertiesDropdownHover(false);
                    // Only close if it wasn't opened by click
                    if (!propertiesDropdownOpen) {
