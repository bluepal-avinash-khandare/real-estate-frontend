...
//       try {
//         setLoading(true);
//         // Use JWT-based getUserProfile instead of getUserById
...
//     try {
//       // Use JWT-based update instead of requiring user ID
...
const Profile = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // Get the user ID from either the user object or the userProfile
      ...
      try {
        // Update the user with a longer timeout
        const updateResponse = await updateUser(userId, values, { timeout: 30000 }); // 30 seconds timeout
        console.log('Update response:', updateResponse);
        
        // Refresh the profile data to get the latest from the server
        await refreshUserProfile();
        
        // Update the AuthContext with the latest user data
        updateAuthUser(userProfile);
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        toast.error('The server is taking too long to respond. Please check your connection and try again.');
      } else {
                ...
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                  <div className="flex items-start">
                    <div className="flex-shrink-0 mt-1">
                      <div className="w-10 h-10 rounded-full bg-[#16A085]/20 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#16A085]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.435.74a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg>
                      </div>
                    </div>
                    <div className="ml-4">
