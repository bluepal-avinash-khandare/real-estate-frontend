...
import { useNavigate } from 'react-router-dom';
import { AuthContext } from '../../contexts/AuthContext';
// Import the components you provided/referenced
import SubscriptionManagement from '../subscriptions/SubscriptionManagement'; 
import SubscriptionForm from '../subscriptions/SubscriptionForm'; // Corrected path based on your provided component name

const AdminDashboard = () => {
    const { signOut, user } = useContext(AuthContext);
    const navigate = useNavigate();

    // State to manage the dynamic view in the dashboard
    // '' = default dashboard, 'add' = SubscriptionForm, 'list' = SubscriptionManagement
    const [currentView, setCurrentView] = useState('');
    
    // Key to force refresh SubscriptionManagement when an action happens
    const [planUpdateKey, setPlanUpdateKey] = useState(0); 
    ...
    // Handler for the "See Subscription Plans" button
    const handleSeePlansClick = () => {
        setCurrentView('list');
    };

    // Handler for the "Add Subscription Plan" button (on dashboard or inside list)
    const handleAddPlanClick = () => {
        setCurrentView('add');
    };

    // Called after a plan is successfully added/updated
    const handlePlanSuccess = () => {
        setPlanUpdateKey(prevKey => prevKey + 1); // Force SubscriptionManagement refresh
        setCurrentView('list'); // Switch to the list view
    };

    // Called when the form is cancelled or closed
    const handleFormCancel = () => {
        setCurrentView(''); // Go back to default dashboard view
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
            {/* ... (Existing Welcome Card Content and Stats Cards) ... */}
            <div className="h-48 bg-gradient-to-r from-[#16A085] to-[#2C3E50] relative">
            <div 
              className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-30"
              style={{ backgroundImage: "url('https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80')" }}
            ></div>
            <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
            <div className="absolute inset-0 flex items-center px-8">
              <h2 className="text-2xl font-bold text-white">Welcome, {user?.name || 'Admin'}!</h2>
            </div>
          </div>
        ...
        {/* Dynamic Content Block: Subscription Management or Add Form */}
        {(currentView === 'list' || currentView === 'add') && (
            <div className="bg-white rounded-2xl shadow-xl p-6 mb-8">
                {currentView === 'list' && (
                    <SubscriptionManagement 
                        key={planUpdateKey} 
                        onPlanUpdate={() => setPlanUpdateKey(prev => prev + 1)} // Refresh list on update/delete
                        onAddPlan={handleAddPlanClick} // Button inside list opens the form
                    />
                )}
                
                {/* NOTE: We pass plan={null} to the form to indicate an 'Add' operation */}
                {currentView === 'add' && (
                    <SubscriptionForm 
                        plan={null}
        ...
        {/* Quick Actions (Updated to 2x2 grid with Subscription buttons) */}
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-gray-800">Quick Actions</h3>
            <div className="h-px bg-gray-200 flex-1 mx-4"></div>
          ...
          {/* Using grid-cols-2 as requested for 2x2 layout */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* 3. Add Subscription Plan (Opens the form) */}
            <button 
              onClick={handleAddPlanClick}
              className={`rounded-xl p-6 text-left hover:shadow-md transition-all border group ${currentView === 'add' ? 'bg-green-500/10 border-green-500 shadow-lg' : 'bg-gradient-to-br from-gray-50 to-gray-100 border-gray-200'}`}
            >
              <div className="flex items-start">
                ...
                <div className="ml-4">
                  <h4 className="font-bold text-gray-800 group-hover:text-green-600 transition-colors">Add Subscription Plan</h4>
                  <p className="text-sm text-gray-600 mt-1">Create a new subscription tier</p>
                </div>
              </div>
            </button>
            
            {/* 4. See Subscription Plans (Opens the list/management view) */}
            <button 
              onClick={handleSeePlansClick}
              className={`rounded-xl p-6 text-left hover:shadow-md transition-all border group ${currentView === 'list' ? 'bg-blue-500/10 border-blue-500 shadow-lg' : 'bg-gradient-to-br from-gray-50 to-gray-100 border-gray-200'}`}
            >
              <div className="flex items-start">
                ...
                <div className="ml-4">
                  <h4 className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors">See Subscription Plans</h4>
                  <p className="text-sm text-gray-600 mt-1">View, edit, and delete existing plans</p>
                </div>
              </div>
            </button>
