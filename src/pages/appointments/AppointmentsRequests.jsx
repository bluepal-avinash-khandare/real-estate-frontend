...
const AppointmentsRequests = () => {
  ...
  const getCustomerName = (appointment) => {
    return appointment.customerDetails || 'Unknown Customer';
  };

  // Helper function to extract email
  const getCustomerEmail = (appointment) => {
    return appointment.createdBy || 'No email';
  };
  ...
  const getFormattedTime = (appointment) => {
    if (appointment.createdAt) {
      const date = new Date(appointment.createdAt);
      return !isNaN(date.getTime()) 
  ...
  const headers = ['ID', 'Name', 'Email', 'Property', 'Date', 'Time', 'Status', 'Actions'];

  const rows = requests.map((req) => {
    const customerName = getCustomerName(req);
    const customerEmail = getCustomerEmail(req);
    const propertyTitle = req.propertyReference || 'Unknown Property';
    const formattedDate = getFormattedDate(req);
    const formattedTime = getFormattedTime(req);

    // Get customer ID for chat functionality - using the same logic as before
    const customerId = req.customer?.id || 
                      req.customerDetails?.id || 
                      req.customerId || 
                      req.buyer?.id || 
